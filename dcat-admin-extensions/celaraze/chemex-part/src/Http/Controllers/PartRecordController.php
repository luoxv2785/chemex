        ...
        return Show::make($id, new PartRecord(['category', 'vendor', 'channel', 'device', 'depreciation']), function (Show $show) {
            ...
            $show->field('expiration_left_days')->as(function () {
                ...
                if (!empty($part_record)) {
                    $depreciation_rule_id = \App\Support\Support::getDepreciationRuleId($part_record);
                    return \App\Support\Support::depreciationPrice($this->price, $this->purchased, $depreciation_rule_id);
                }
            });
