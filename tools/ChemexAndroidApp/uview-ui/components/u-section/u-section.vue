<template>
    <view class="u-section">
        <view :class="{
            ...
            <view v-if="showLine" :style="[lineStyle]" class="u-section__title__icon-wrap u-flex">
                <u-icon :color="lineColor ? lineColor : color" :size="fontSize * 1.25" bold name="column-line"
                        top="0"></u-icon>
            </view>
            <text class="u-flex u-section__title__text">{{ title }}</text>
        </view>
        <view v-if="right || $slots.right" :style="{
			color: subColor
		}" class="u-section__right-info" @tap="rightClick">
            <slot v-if="$slots.right" name="right"/>
            <block v-else>
                {{ subTitle }}
                <view v-if="arrow" class="u-section__right-info__icon-arrow u-flex">
                    <u-icon :color="subColor" name="arrow-right" size="24"></u-icon>
                </view>
            </block>
        </view>
    </view>
