<template>
    <view :style="[wrapStyle]" class="u-avatar" @tap="click">
        <image
            v-if="!uText && avatar"
            :mode="imgMode"
            :src="avatar"
            ...
            @error="loadError"
        ></image>
        <text v-else-if="uText" :style="{
			fontSize: '38rpx'
		}" class="u-line-1">{{ uText }}
        </text>
        <slot v-else></slot>
        <view v-if="showSex" :class="['u-avatar__sex--' + sexIcon]" :style="[uSexStyle]" class="u-avatar__sex">
            <u-icon :name="sexIcon" size="20"></u-icon>
        </view>
        <view v-if="showLevel" :style="[uLevelStyle]" class="u-avatar__level">
