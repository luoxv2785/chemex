    ...
    <view :class="{'u-border-top': borderTop, 'u-border-bottom': borderBottom }" class="u-field">
        <view :class="[type == 'textarea' ? 'u-textarea-inner' : '', 'u-label-postion-' + labelPosition]"
            ...
            <view :class="[required ? 'u-required' : '']" :style="{
                ...
                <slot name="icon"></slot>
                <text :class="[this.$slots.icon || icon ? 'u-label-left-gap' : '']" class="u-label-text">{{
                        label
                    }}
                </text>
            </view>
            <view class="fild-body">
                <view :style="[inputWrapStyle]" class="u-flex-1 u-flex">
					<textarea v-if="type == 'textarea'" :autoHeight="autoHeight" :disabled="disabled"
                              :fixed="fixed"
                ...
                <u-icon v-if="clearable && value != '' && focused" :size="clearSize" class="u-clear-icon"
                        color="#c0c4cc" name="close-circle-fill" @click="onClear"/>
                <view class="u-button-wrap">
                    <slot name="right"/>
                </view>
                <u-icon v-if="rightIcon" :name="rightIcon" :style="[rightIconStyle]" class="u-arror-right"
                        color="#c0c4cc" size="26" @click="rightIconClick"/>
            </view>
        </view>
