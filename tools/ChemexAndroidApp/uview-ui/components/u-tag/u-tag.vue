<template>
    <view v-if="show" :class="[
		...
		'u-mode-' + mode + '-' + type
          ...
          :style="[customStyle]" class="u-tag" @tap="clickTag">
        {{ text }}
        <view class="u-icon-wrap" @tap.stop>
            <u-icon v-if="closeable" :color="closeIconColor" :style="[iconStyle]" class="u-close-icon"
                    name="close" size="22" @click="close"></u-icon>
        </view>
    </view>
</template>
...
.u-shape-circleRight {
    border-radius: 0 100 rpx 100 rpx 0;
}

.u-shape-circleLeft {
    border-radius: 100 rpx 0 0 100 rpx;
}
